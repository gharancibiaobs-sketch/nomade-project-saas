  ...
  return (
    <article className="group flex flex-col gap-5 bg-transparent">
      <div className="relative overflow-hidden bg-[#F0EEE9]">
        ...
        />
        {product.descripcion && (
          <div className="pointer-events-none absolute inset-0 flex items-end bg-[#252321]/0 p-5 opacity-0 transition duration-500 group-hover:bg-[#252321]/55 group-hover:opacity-100">
      ...
      <div className="flex items-center gap-3 font-sans text-[9pt] uppercase tracking-[0.16em] text-[#6B655F]">
        {hasOffer ? (
          <>
            <span className="text-[#252321]">{formatCurrency(offerPrice)}</span>
            <span className="line-through opacity-40">{formatCurrency(originalPrice)}</span>
          </>
        ) : (
          <span className="text-[#252321]">{formatCurrency(originalPrice)}</span>
        )}
      </div>
    </article>
  );
